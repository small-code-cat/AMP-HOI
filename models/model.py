...
from datasets.hico_categories import HICO_ACTIONS
...
import models.model_improvements as model_improvements
...
class HOIVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        heads: int,
        output_dim: int,
        enable_resnet50: bool,
        enable_visual_lambda1: bool,
        enable_visual_lambda2: bool,
        lamb: int,
        enable_visual_prompt: bool,
        ...
        self.enable_visual_lambda1 = enable_visual_lambda1
        self.enable_visual_lambda2 = enable_visual_lambda2
        self.lamb = lamb

        self.enable_resnet50 = enable_resnet50
        if self.enable_resnet50:
            ...
            self.resnet50.fc = nn.Sequential(
                ...
                nn.Dropout(p=0.5),
                nn.Linear(num_ftrs//2, 768)
            )

        self.enable_visual_prompt = enable_visual_prompt
        self.enable_unified_prompt = enable_unified_prompt
        if self.enable_visual_prompt:
            self.my_visual_prompt = nn.Parameter(torch.zeros(10, width))
        self.my_prompt_dropout = nn.Dropout(0.1)
    ...
    def incorporate_prompt(self, x, visual_prompt):
        # combine prompt embeddings with image-patch embeddings
        B = x.shape[0]
        # after CLS token, all before image patches
        x = torch.cat((
                x[:, :1, :],
                self.my_prompt_dropout(visual_prompt.expand(B, -1, -1)),
                x[:, 1:, :]
            ), dim=1)
    ...
    def forward(self, image: torch.Tensor, visual_prompt=None, mask: torch.Tensor = None):
        if self.enable_resnet50:
            local_image_features = self.resnet50(image)
        ...
        if mask is not None:
            ...
            if self.enable_unified_prompt:
                mask_flatten = torch.cat([torch.ones((mask_flatten.shape[0], 1), dtype=mask.dtype, device=mask.device), torch.zeros((mask_flatten.shape[0], visual_prompt.shape[0]), dtype=mask.dtype, device=mask.device), mask_flatten], dim=-1)
            elif self.enable_visual_prompt:
                mask_flatten = torch.cat([torch.ones((mask_flatten.shape[0], 1), dtype=mask.dtype, device=mask.device),
                                          torch.zeros((mask_flatten.shape[0], self.my_visual_prompt.shape[0]),
                                                      dtype=mask.dtype, device=mask.device), mask_flatten], dim=-1)
            else:
        ...
        if self.enable_unified_prompt:
            image = self.incorporate_prompt(image, visual_prompt)
        elif self.enable_visual_prompt:
            image = self.incorporate_prompt(image, self.my_visual_prompt)
        hoi = hoi + self.hoi_pos_embed

        image = self.ln_pre(image) # bs, change, 768
        hoi = self.ln_pre(hoi)
        ...
        if self.enable_resnet50:
            if self.enable_visual_lambda1:
                image_features = self.lamb*image_features + (1-self.lamb)*local_image_features
            else:
                image_features += local_image_features

            if self.enable_visual_lambda2:
                hoi_features = self.lamb * hoi_features + ((1 - self.lamb) * local_image_features).unsqueeze(1)
            else:
                hoi_features += local_image_features.unsqueeze(1)
        ...
        if self.enable_dec:
            ...
            if self.enable_unified_prompt:
                memory = image[1 + visual_prompt.shape[0]:]
            elif self.enable_visual_prompt:
                memory = image[1 + self.my_visual_prompt.shape[0]:]
            else:
                memory = image[1:]

            if self.enable_unified_prompt:
                memory_key_padding_mask=mask_flatten[:, 1 + visual_prompt.shape[0]:]
            elif self.enable_visual_prompt:
                memory_key_padding_mask=mask_flatten[:, 1 + self.my_visual_prompt.shape[0]:]
            else:
                memory_key_padding_mask=mask_flatten[:, 1:]

            hidden = self.bbox_head(
                tgt=hoi,
                tgt_mask=self.hoi_parser_attn_mask[1:, 1:].to(hoi.device), # exclude [CLS]
                query_pos=self.hoi_pos_embed[:, None, :],
                memory=memory, # exclude [CLS]
                memory_key_padding_mask=memory_key_padding_mask, # exclude [CLS]
                pos=patch_pos)
...
class HOIDetector(nn.Module):
    def __init__(
        ...
        enable_resnet50:bool,
        enable_gru:bool,
        enable_text_lambda: bool,
        enable_visual_lambda1: bool,
        enable_visual_lambda2: bool,
        lamb:int,
        enable_visual_prompt:bool,
        enable_unified_prompt:bool,
        prefix_length: int = 8,
        conjun_length: int = 4,
    ):
        super().__init__()
        ...
        self.enable_unified_prompt = enable_unified_prompt
        if self.enable_unified_prompt:
            self.my_unified_prompt = nn.Parameter(torch.empty(prefix_length+conjun_length, 1024))
            self.my_unified_to_text = nn.Sequential(
                nn.Linear(1024, 1024*2),
                nn.GELU(),
                nn.Linear(1024*2, transformer_width),
            )
            self.my_unified_to_visual = nn.Sequential(
                nn.Linear(1024, 1024 * 2),
                nn.GELU(),
                nn.Linear(1024 * 2, vision_width),
            )
        ...
        self.visual = HOIVisionTransformer(
            ...
            enable_resnet50=enable_resnet50,
            enable_visual_lambda1=enable_visual_lambda1,
            enable_visual_lambda2=enable_visual_lambda2,
            lamb=lamb,
            enable_visual_prompt=enable_visual_prompt,
            enable_unified_prompt=enable_unified_prompt,
        )
        ...
        self.enable_text_lambda = enable_text_lambda
        self.lamb = lamb
    ...
    def initialize_parameters(self):
        ...
        if self.enable_unified_prompt:
            nn.init.normal_(self.my_unified_prompt, std=0.01)
        else:
    ...
    def encode_image(self, image, visual_prompt=None, image_mask=None):
        return self.visual(image.type(self.dtype), visual_prompt, image_mask)

    def encode_text(self, text):
        # x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        x, eot_indices = self.text_to_embedding(text)
        if self.enable_gru:
            local_text_features = self.gru(x)[0][:, -1, :]
        ...
        if self.enable_gru:
            if self.enable_text_lambda:
                x = self.lamb*x + (1-self.lamb)*local_text_features
            else:
                x += local_text_features
    ...
    def text_to_embedding(self, text):
        ...
        """
        if self.enable_unified_prompt:
            text_prompt = self.my_unified_to_text(self.my_unified_prompt)
            hoi_prefix = text_prompt[:self.prefix_length, :]
            hoi_conjun = text_prompt[self.prefix_length:, :]
        ...
        for action_token, object_token in text:
            ...
            if self.enable_unified_prompt:
                full_token_embedding = torch.cat([
                    token_embedding[0:1, :], hoi_prefix, token_embedding[1:len(action_token), :],
                    hoi_conjun, token_embedding[len(action_token):, :]], dim=0)
            else:
                full_token_embedding = torch.cat([
    ...
    def calc_cross_attention(self, cls1, cls2):
        # 但是原框架的前提是第一个维度也就是n需要一样
        # 经过改动后可以做到维度不一致也可以运行
        cls1_repeat = torch.cat([cls1.repeat(cls2.shape[0] // cls1.shape[0], 1),
                                    cls1[:cls2.shape[0] % cls1.shape[0], :]], dim=0)

        cross_input = torch.stack((cls1_repeat + self.pos_emb1D[0, :], cls2 + self.pos_emb1D[1, :]),
                                     dim=1)  # (n, 2, 512)

        attn_result = self.cross_attention(cross_input)
        return attn_result[:cls1.shape[0], 0, :], attn_result[:, 1, :]

    def forward(self, image, text, image_mask=None):
        visual_prompt = self.my_unified_to_visual(self.my_unified_prompt) if self.enable_unified_prompt else None
        # vision encoder
        vision_outputs = self.encode_image(image, visual_prompt, image_mask)
...
def get_action_dict():
    action_list = [i['name'] for i in HICO_ACTIONS]
    action_token_list = [tuple(_tokenizer.encode(action.replace("_", " "))) for action in action_list]
    token_dict = dict(zip(action_list,action_token_list))
    with open('verb.json', 'r') as f:
        verb_dict = json.load(f)
    action_dict = {token_dict[i]:verb_dict[i] for i in action_list}
    return action_dict
...
def build_model(args):
    ...
    model = HOIDetector(
        ...
        enable_resnet50=args.enable_resnet50,
        enable_gru=args.enable_gru,
        enable_text_lambda=args.enable_text_lambda,
        enable_visual_lambda1=args.enable_visual_lambda1,
        enable_visual_lambda2=args.enable_visual_lambda2,
        lamb=args.lamb,
        enable_visual_prompt=args.enable_visual_prompt,
        enable_unified_prompt=args.enable_unified_prompt,
    )
